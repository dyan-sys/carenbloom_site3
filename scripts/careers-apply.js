(function () {
  ...
  // Auto-roll: closing date is always end of current month (or next month if already past)
  const getCloseDate = () => {
    const now = new Date();
    let endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59);
    if (now > endOfMonth) endOfMonth = new Date(now.getFullYear(), now.getMonth() + 2, 0, 23, 59, 59);
    return endOfMonth;
  };
  const closeAt = getCloseDate();
  const monthNames = ["January","February","March","April","May","June","July","August","September","October","November","December"];
  const closeLabel = `${monthNames[closeAt.getMonth()]} ${closeAt.getDate()}`;
  const closeLine = document.querySelector(".role-close-line span");
  if (closeLine) closeLine.textContent = `Closes ${closeLabel}`;
  const closeApplyLink = document.querySelector(".role-close-apply");
  if (closeApplyLink) closeApplyLink.setAttribute("aria-label", `Apply before role closes ${closeLabel}, ${closeAt.getFullYear()}`);
  if (countdown) countdown.setAttribute("datetime", closeAt.toISOString());
  ...
  if (form) {
    form.addEventListener("submit", async (event) => {
      ...
      if (!form.reportValidity()) {
        const missingFile = [...form.querySelectorAll("input[type=file][required]")].find((input) => !input.files?.length);
        if (missingFile) {
          const label = missingFile.closest("label")?.querySelector("span")?.textContent || "A required file";
          setStatus(`${label.replace(/\*$/, "")} is required.`, "error");
          missingFile.closest("label")?.scrollIntoView({ behavior: "smooth", block: "center" });
        }
        return;
      }
      ...
      if (oversizedFile) {
        setStatus("Keep file uploads under 4 MB each.", "error");
        return;
      }
      ...
      try {
        ...
        if (response.status === 413) throw new Error("Your files are too large. Keep each file under 4 MB.");
        ...
        if (typeof fbq === "function") fbq("track", "Lead", { content_name: role.title });
        form.reset();
      } catch (error) {
